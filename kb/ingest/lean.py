...
Usage:
    python kb/ingest/lean.py [--dry-run] [--limit N]
...
def run(
    ...
    """Ingest Lean theorems in-process.  Returns 0 on success."""
    # Post-commit hook: skip summarize for speed; human ingestion runs it
    if files and not summarize_only:
        no_summarize = True
    ...
    for project, grp in groupby(sorted_pairs, key=lambda x: x[2]):
        ...
        print(f"\nParsing {len(file_args)} files [{project}] with {workers} workers...")
        with ProcessPoolExecutor(max_workers=workers) as pool:
            ...
            try:
                ...
                if pending and not (limit and total >= limit):
    ...
    if not no_summarize and new_by_file and llm is not None:
