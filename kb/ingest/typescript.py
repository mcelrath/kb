...
# Allow standalone execution (e.g. from post-commit hooks): ensure the package root is on sys.path.
...
def run(
    ...
    """Ingest TypeScript symbols in-process.  Returns 0 on success, 1 on fatal error."""
    if db_path is None:
    ...
    if files:
        ...
        file_list = collect_ts_files(root)
    ...
    print(f"Processing {len(file_list)} TypeScript files (dry_run={dry_run})", file=sys.stderr)
